...
import actrix.transforms.functional
import torch
...
class UniformTemporalSubsample(torch.nn.Module):
    """
    ``nn.Module`` wrapper for ``actrix.transforms.functional.uniform_temporal_subsample``.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return actrix.transforms.functional.uniform_temporal_subsample(
...
class UniformTemporalSubsampleRepeated(torch.nn.Module):
    ...
    ``actrix.transforms.functional.uniform_temporal_subsample_repeated``.
    """
    ...
    def forward(self, x: torch.Tensor):
        ...
        Args:
            x (torch.Tensor): video tensor with shape (C, T, H, W).
        """
        return actrix.transforms.functional.uniform_temporal_subsample_repeated(
...
class ShortSideScale(torch.nn.Module):
    """
    ``nn.Module`` wrapper for ``actrix.transforms.functional.short_side_scale``.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): video tensor with shape (C, T, H, W).
        """
        return actrix.transforms.functional.short_side_scale(
...
class RandomShortSideScale(torch.nn.Module):
    """
    ``nn.Module`` wrapper for ``actrix.transforms.functional.short_side_scale``. The size
...
class UniformCropVideo(torch.nn.Module):
    """
    ``nn.Module`` wrapper for ``actrix.transforms.functional.uniform_crop``.
    """
    ...
    def __call__(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        Args:
            x (Dict[str, torch.Tensor]): video clip dict.
        """
        x[self._video_key] = actrix.transforms.functional.uniform_crop(
...
class RandomResizedCrop(torch.nn.Module):
    """
    ``nn.Module`` wrapper for ``actrix.transforms.functional.random_resized_crop``.
    """
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return actrix.transforms.functional.random_resized_crop(
            x,
            self._target_height,
            self._target_width,
            self._scale,
...
class Div255(torch.nn.Module):
    """
    ``nn.Module`` wrapper for ``actrix.transforms.functional.div_255``.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torchvision.transforms.Lambda(
            actrix.transforms.functional.div_255
        )(x)
